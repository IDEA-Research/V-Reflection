...
#SBATCH --output=/comp_robot/zhoujiazhou/projects/Active-Coconut/logs/evaluation_7b_dit_recon_%j.txt

# ============================================================================
# Evaluation Script for DiT Reconstruction Model
# Tests all checkpoints with step4 and step8 to compare LVR thinking steps
# Supports DiT image generation during inference
# ============================================================================

# Base checkpoint directory for dit_recon model
BASE_CHECKPOINT_DIR="${BASE_CHECKPOINT_DIR:-/comp_robot/zhoujiazhou/projects/Active-Coconut/result/dit_recon/SFT_dit_recon_steps2500_b4_dit1.0_resampler0.1_acc8_LATENT8}"
...
# Evaluation step list - lvr_steps=N produces N LVR mode forward calls
# For DiT image generation with 8 latent tokens, use lvr_steps=8
EVAL_STEP_LIST="${EVAL_STEP_LIST:-4,8,12}"
...
FORCE_RE_EVALUATE="${FORCE_RE_EVALUATE:-1}"
export FORCE_RE_EVALUATE

# ============================================================================
# DiT Image Generation Options
# ============================================================================
# Default: enable DiT image generation during inference.
# Use DIT_GENERATE_IMAGES=0 to disable in a run.
# Generated images will be saved to: evaluation/results/{benchmark}/dit_generated/{run_name}/{checkpoint_num}/
DIT_GENERATE_IMAGES="${DIT_GENERATE_IMAGES:-1}"
export DIT_GENERATE_IMAGES

# Number of DiT denoising steps (default: 20, more steps = better quality but slower)
DIT_NUM_INFERENCE_STEPS="${DIT_NUM_INFERENCE_STEPS:-20}"
export DIT_NUM_INFERENCE_STEPS

# Enable DiT debug output (set to 1 to see detailed DiT generation logs)
DIT_DEBUG="${DIT_DEBUG:-1}"
export DIT_DEBUG
...
echo ""
echo "============================================================================"
echo "Step 2: Starting Evaluation for DiT Reconstruction Model"
echo "============================================================================"
echo "Base Directory: $BASE_CHECKPOINT_DIR"
echo "Checkpoints: ${CHECKPOINT_STEPS[*]}"
echo "Evaluation Steps: $EVAL_STEP_LIST"
echo "DiT Image Generation: $([ "$DIT_GENERATE_IMAGES" = "1" ] && echo "Enabled (steps=$DIT_NUM_INFERENCE_STEPS)" || echo "Disabled")"
echo "============================================================================"
...
run_evaluation() {
    ...
    # Set up DiT save directory if enabled
    if [ "$DIT_GENERATE_IMAGES" = "1" ]; then
        local run_name=$(basename "$(dirname "$checkpoint_path")")
        export DIT_SAVE_DIR="${PROJECT_ROOT}/evaluation/results/dit_generated/${run_name}/checkpoint-${checkpoint_step}"
        mkdir -p "$DIT_SAVE_DIR"
        echo "DiT images will be saved to: $DIT_SAVE_DIR"
    fi
...
echo ""
echo "============================================================================"
echo "DiT Reconstruction Evaluation Summary"
echo "============================================================================"
...
if [ "$DIT_GENERATE_IMAGES" = "1" ]; then
    echo ""
    echo "DiT generated images saved to:"
    echo "  ${PROJECT_ROOT}/evaluation/results/dit_generated/"
fi
