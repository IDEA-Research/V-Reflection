...
#SBATCH --output=logs/evaluation_7b_SFT_steps8_%j.txt

# ============================================================================
# Configuration - Model Settings
# ============================================================================
# Base checkpoint directory - can be overridden via environment variable (set by sbatch training scripts)
# Run from project root; set BASE_CHECKPOINT_DIR to your checkpoint directory (required when run standalone)
BASE_CHECKPOINT_DIR="${BASE_CHECKPOINT_DIR:-}"

if [ -z "$BASE_CHECKPOINT_DIR" ]; then
    echo "Error: BASE_CHECKPOINT_DIR is not set. Set it to your checkpoint directory, e.g.:"
    echo "  export BASE_CHECKPOINT_DIR=result/box_resampler/SFT_box_resampler_steps2500_.../checkpoint-2400"
    echo "Or run from a training sbatch script which sets it automatically."
    exit 1
fi
...
# Evaluation step list (override via env, e.g. EVAL_STEP_LIST=4 from sbatch scripts)
...
# Under sbatch: use SLURM_SUBMIT_DIR; else derive from script location
if [ -n "${SLURM_SUBMIT_DIR}" ]; then
    PROJECT_ROOT="${SLURM_SUBMIT_DIR}"
elif [ -n "${PROJECT_ROOT}" ]; then
    :  # Already set by parent (e.g. training script)
else
...
run_evaluation() {
    ...
    # evaluation.py removes result prefix from path for run_name
    ...
    if [ -f "$MERGE_SCRIPT" ]; then
        ...
        for step in "${STEP_ARRAY[@]}"; do
            ...
            # Merge HRBench4K, HRBench8K, MME-RealWorld-Lite
            for ds in HRBench4K HRBench8K MME-RealWorld-Lite; do
                ds_result_dir="${PROJECT_ROOT}/evaluation/results/${ds}/decoding_by_steps/${run_name}"
                if [ -d "$ds_result_dir" ]; then
                    echo ""
                    echo "Merging ${ds} process results for checkpoint-${checkpoint_step}, step ${step}..."
                    python "$MERGE_SCRIPT" "$ds_result_dir" "$ds" "$checkpoint_num" "$step" || true
                fi
            done
        done
    else
    ...
    # Calculate accuracy for HRBench4K, HRBench8K, MME-RealWorld-Lite
    for ds in HRBench4K HRBench8K MME-RealWorld-Lite; do
        local ds_json_path="${PROJECT_ROOT}/evaluation/results/${ds}/decoding_by_steps/${run_name}/ck-${checkpoint_num}-step4.json"
        if [ -f "$ds_json_path" ]; then
            echo ""
            echo "Calculating accuracy for ${ds} dataset (checkpoint-${checkpoint_step})..."
            echo "File: $ds_json_path"
            python "$ACCURACY_SCRIPT" "$ds_json_path"
            if [ $? -ne 0 ]; then
                echo "Warning: Failed to calculate accuracy for ${ds} dataset (checkpoint-${checkpoint_step})"
            fi
        else
            echo "Warning: ${ds} result file not found: $ds_json_path"
        fi
    done
