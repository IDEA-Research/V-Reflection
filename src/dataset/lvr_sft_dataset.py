...
from .data_utils import get_image_info, get_video_info, llava_to_openai_lvr, pad_sequence, map_image_path
import numpy as np
...
class SupervisedDatasetLVR(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        # Handle cases where bboxes might not be present in the data
        bboxes = sources.get('bboxes', [])
        if bboxes is None:
            bboxes = []
        
        # If no bboxes provided, create empty token indices list
        if len(bboxes) == 0:
            # Return empty list of token indices for each image
            lvr_token_idxs_list_manual = [[] for _ in images] if images else []
        else:
            '''
